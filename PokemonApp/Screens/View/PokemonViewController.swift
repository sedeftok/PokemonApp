...
extension PokemonViewController: PokemonViewModelOutput {
    func updateView(state: PokemonListViewModelState) {
        switch state {
        case .showPokemonList(let pokemonCellViewModel):
            delegate?.update(cellViewModel: pokemonCellViewModel)
            dataSource?.update(cellViewModel: pokemonCellViewModel)
